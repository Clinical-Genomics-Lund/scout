...
def variant_verification(
    ...
    hg38_str = subprocess.check_output(
        [
            "/data/bnf/scripts/hg38_pos.pl",
            str(variant_obj["chromosome"]),
            str(variant_obj["position"]),
        ]
    ).decode("utf-8")
    ...
    ncbi_link = (
        ...
        + ncbi_chr[hg38_chr]
        + "?report=fasta&from="
        + str(int(hg38_pos) - 500)
        + "&to="
        + str(int(hg38_pos) + 500)
        + '">NCBI genomic region</a>'
    )
    thermo_link = (
        '<a href="https://www.thermofisher.com/order/genome-database/searchResults?searchMode=keyword&CID=&ICID=&productTypeSelect=ceprimer&targetTypeSelect=ceprimer_all&alternateTargetTypeSelect=&alternateProductTypeSelect=&originalCount=0&species=Homo+sapiens&otherSpecies=&additionalFilter=ceprimer-human-exome&keyword=&sequenceInput=&selectedInputType=&chromosome='
        + hg38_chr
        + "&chromStart="
        + hg38_pos
        + "&chromStop="
        + hg38_pos
        + '&vcfUpload=&multiChromoSome=&batchText=&batchUpload=&sequenceUpload=&multiSequence=&multiSequenceNames=&priorSearchTerms=%28NR%29">Order primers from ThermoFisher</a>'
    )
