...
def test_comment(real_variant_database, institute_obj, case_obj, user_obj):
    adapter = real_variant_database
    content = 'specific comment for a variant'
    # GIVEN a populated database with variants
    assert adapter.variant_collection.find().count() > 0

    # And an empty event database collection
    assert adapter.event_collection.find().count() == 0

    # Get a random variant from variants collection
    variant = adapter.variant_collection.find_one()
